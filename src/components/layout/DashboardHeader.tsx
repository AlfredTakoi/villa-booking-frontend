...
const displayAssetUrl = (url: string) => url.startsWith('//') && typeof window !== 'undefined'
...
export default function DashboardHeader({ onToggleSidebar }: DashboardHeaderProps) {
  ...
  const [dashboardLogo, setDashboardLogo] = useState(localAsset('Logo-Kemenkes.png'))

  useEffect(() => {
    fetch(buildApiUrl('/api/settings'))
      .then((response) => response.json())
      .then((payload) => {
        const logo = payload?.settings?.dashboard_header_logo
        if (payload?.success && logo) setDashboardLogo(displayAssetUrl(logo))
      })
      .catch(() => undefined)
  }, [])
