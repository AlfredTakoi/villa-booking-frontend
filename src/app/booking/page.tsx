...
function BookingPaymentContent() {
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-6">
          <Link
            href="/"
            className="inline-flex items-center gap-1.5 text-xs text-charcoal-800/60 hover:text-gold-600 transition-colors uppercase tracking-wider font-semibold"
          >
        ...
        <div className="text-center max-w-2xl mx-auto mb-10">
          <span className="text-xs font-bold uppercase tracking-[0.2em] text-gold-600 bg-gold-100 px-3 py-1 rounded-full">
            Langkah 2: Pembayaran & Konfirmasi
          </span>
          <h1 className="font-serif text-3xl sm:text-4xl font-bold text-charcoal-900 mt-3 mb-3">
        ...
        {/* Card Input Kode Reservasi */}
        <div className="bg-white p-6 sm:p-10 rounded-3xl border border-sand-300 shadow-sm max-w-xl mx-auto space-y-6">
          <div className="w-14 h-14 rounded-2xl bg-gold-100 text-gold-600 flex items-center justify-center mx-auto">
          ...
          <form onSubmit={handleSearchCode} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-gold-500 to-gold-600 hover:from-gold-400 hover:to-gold-500 text-charcoal-950 py-3.5 rounded-full font-bold uppercase text-xs tracking-wider flex items-center justify-center gap-2 shadow-lg shadow-gold-500/20 transition-all"
            >
          ...
          <div className="pt-6 border-t border-sand-200 text-center space-y-3">
            <p className="text-xs text-charcoal-800/70">
              Belum melakukan pemesanan tanggal?
            </p>
            <button
              type="button"
              onClick={() => setIsModalOpen(true)}
              className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-gold-600 hover:text-gold-700 bg-gold-50 hover:bg-gold-100 px-5 py-2.5 rounded-full transition-colors"
            >
              <Calendar className="w-4 h-4" />
              <span>Buka Kalender Reservasi Sekarang</span>
            </button>
          </div>
        </div>
  ...
  // KONDISI 2: Loading data reservasi
  if (isLoading) {
    return (
  ...
  // KONDISI 3: Reservasi tidak ditemukan
      ...
      <div className="max-w-xl mx-auto px-4 py-20 text-center">
        ...
        <div className="flex justify-center gap-3">
          ...
          <button
            type="button"
            onClick={() => setIsModalOpen(true)}
            className="bg-gold-500 text-charcoal-950 px-6 py-3 rounded-full text-xs font-bold uppercase tracking-wider"
          >
            Buat Reservasi Baru
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
          <div>
            ...
            <span className="text-xs text-white/60 block mt-2">
              Atas Nama: <strong className="text-white">{bookingData.guest?.name || '-'}</strong> ({bookingData.guest?.phone || '-'})
            </span>
          </div>
          ...
          <div className="sm:text-right border-t sm:border-t-0 sm:border-l border-white/10 pt-4 sm:pt-0 sm:pl-8">
            ...
            <span className="text-xs text-white/60 block mt-1.5">
              Masa Inap: {bookingData.check_in} &mdash; {bookingData.check_out} ({bookingData.total_nights} Malam){villa.allow_guest_selection && bookingData.total_guests ? ` \u2022 ${bookingData.total_guests} Tamu` : ''}
            </span>
          </div>
        </div>
      </div>

      {/* Konten Status / Upload */}
      {isConfirmed ? (
        <div className="bg-white p-8 sm:p-10 rounded-3xl border border-emerald-200 shadow-sm text-center space-y-6">
          ...
          <div className="max-w-md mx-auto space-y-2">
            ...
            <p className="text-sm text-charcoal-800/70">
              Invoice resmi telah diterbitkan. Anda dapat mengunduh dokumen invoice PDF di bawah ini sebagai bukti reservasi resmi.
            </p>
          </div>
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-sand-300 shadow-sm space-y-6">
              ...
              <div className="p-4 bg-gold-50 rounded-2xl border border-gold-200 text-xs text-gold-900 space-y-1">
                ...
                <p className="text-[11px] text-gold-800/90 leading-relaxed">
                  Cantumkan kode reservasi <strong>{bookingData.booking_code}</strong> pada berita transfer untuk mempercepat proses verifikasi tim admin.
                </p>
              </div>
                ...
                <div className="p-6 bg-emerald-50 border border-emerald-200 rounded-2xl text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-800/80 mt-1 leading-relaxed">
                      Tim reservasi {villa.name || 'Villa Casa Anandefa'} sedang memeriksa transfer Anda. Status konfirmasi dan invoice resmi akan otomatis dikirimkan ke WhatsApp pemesan.
                    </p>
                  </div>
                      ...
                      <div className="mt-4 p-4 bg-white rounded-2xl border border-emerald-200 shadow-sm text-left">
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-xs font-bold uppercase tracking-wider text-charcoal-800 flex items-center gap-1.5">
                            ...
                            Dokumen / Foto Bukti yang Dikirim
                          </span>
                          <a
                            href={proofSrc}
                            target="_blank"
...
export default function BookingPage() {
  return (
    <main className="min-h-screen bg-sand-50 pt-20">
      <Navbar />
      <Suspense fallback={<div className="text-center py-20 text-muted">Memuat data pembayaran...</div>}>
